  ...
  return (
    <div className='space-y-6'>
        <div className='flex items-center justify-between'>
            ...
            <button onClick={addEducation} className='flex items-center gap-2 px-3 py-1 text-sm bg-green-100 text-green-700 rounded hover:bg-purple-200 transition-colors'>
                    ...
                    <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                        ...
                        <div className='grid md:grid-cols-2 gap-3'>
                            ...
                            <input value={education.institution ||""} onChange={(e)=>updateEducation(index, "institution", e.target.value)}
                             type="text" name="" id="" placeholder='institution name' className='px-3 py2 text-sm '/>

                            <input value={education.degree ||""} onChange={(e)=>updateEducation(index, "degree", e.target.value)}
                             type="text" name="" id="" placeholder="degree (e.g., bachelor', master's)" className='px-3 py2 text-sm '/> 

                            <input value={education.field ||""} onChange={(e)=>updateEducation(index, "field", e.target.value)}
                             type="text" name="" id="" placeholder='field of study' className='px-3 py2 text-sm '/> 

                            <input value={education.graduation_date ||""} onChange={(e)=>updateEducation(index, "graduation_date", e.target.value)}
                             type="month" name="" id="" placeholder='company name' className='px-3 py2 text-sm'/>     

                        </div>               

                        <input value={education.gpa ||""} onChange={(e)=>updateEducation(index, "gpa", e.target.value)}
                             type="text" name="" id="" placeholder="gpa (optional)" className='px-3 py2 text-sm '/> 
                    </div>                    
                ))}
            </div>
        )}       
